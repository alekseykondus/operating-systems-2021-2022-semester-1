...
public class Kernel extends Thread {
    ...
    private List<Page> memList = new ArrayList<>();
    ...
    public void init(String commands, String config) {
        ...
        for (i = 0; i < virtPageNum; i++) {
            ...
            for (j = 0; j < virtPageNum; j++) {
                Page tmp_page = (Page) memList.get(j);
        ...
        if (map_count < (virtPageNum + 1) / 2) {
            System.out.println("Popal siuda");
            for (i = 0; i < virtPageNum; i++) {
                Page page = (Page) memList.get(i);
    ...
    public void getPage(int pageNum) {
        Page page = (Page) memList.get(pageNum);
    ...
    public void step() {
        ...
        if (instruct.inst.startsWith("READ")) {
            Page page = (Page) memList.get(Virtual2Physical.pageNum(instruct.addr, virtPageNum, block));
            if (page.physical == -1) {
                if (doFileLog) {
                ...
                PageFault.replacePageRoundRobin(memList, virtPageNum, Virtual2Physical.pageNum(instruct.addr, virtPageNum, block), controlPanel);
